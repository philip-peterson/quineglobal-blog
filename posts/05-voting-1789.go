...
var voting1789 = model.QuinePost{
	...
	Content: []Node{
		...
		P(Text("43% of Americans now identify as politically Independent. Only 28% are Republican. Another 28% are Democrat. The numbers are not ambiguous. If elections reflected reality, we would have an Independent president, or at least a contest that allowed for one. But we don't. Instead, we have a legacy system—first-past-the-post voting—built for simplicity, not representation. Built to prevent feedback. Built to forget.")),

		P(Text("There is a rot that sets in when systems persist too long without iteration. In software, this becomes technical debt. In governance, it becomes misalignment so severe that even consensus is invisible. Voters are told they must choose between two options they do not prefer. Any third option is “spoiler,” “vanity,” “waste.” The will of the majority is not lost—it is structurally excluded.")),

		P(Text("There are ways out. Ranked-choice voting. Approval voting. Basic patches to a system now brittle from neglect. These aren't theoretical; they are mathematically sound, already deployed in cities and countries that understand the stakes. They allow systems to see what is actually there, not just what legacy formats can parse.")),
